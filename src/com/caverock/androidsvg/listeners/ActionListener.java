...
public class ActionListener implements OnTouchListener
{
	...
	public ActionListener(SVGImageView imageView)
	{
		setImageView(imageView);
	}

	public ActionListener(SVGImageView imageView, Context context)
	{
		this(imageView);
		setContext(context);
	}
	...
	{
		if (line.x1 == null) line.x1 = new SVG.Length(0);
		if (line.y1 == null) line.y1 = new SVG.Length(0);
		if (line.x2 == null) line.x2 = new SVG.Length(0);
		if (line.y2 == null) line.y2 = new SVG.Length(0);
	...
	{
		try{mShape = mShape.clone();}catch(CloneNotSupportedException e){e.printStackTrace();}
		return true;
	}
	...
	{
		if (circle.cx == null) circle.cx = new SVG.Length(0);
		if (circle.cy == null) circle.cy = new SVG.Length(0);
		if (circle.r == null)  circle.r  = new SVG.Length(0);
	...
	{
		circle.r.setValue(mRenderer, Math.abs(Math.min(circle.cx.value-x, circle.cx.value-y)));
		mImageView.invalidate();
		return true;
	}
	...
	{
		if (ellipse.cx == null) ellipse.cx = new SVG.Length(0);
		if (ellipse.cy == null) ellipse.cy = new SVG.Length(0);
		if (ellipse.rx == null) ellipse.rx = new SVG.Length(0);
		if (ellipse.ry == null) ellipse.ry = new SVG.Length(0);
	...
	{
		if (path.d == null) path.d = new SVG.PathDefinition();
		path.d.moveTo(x, y);
	...
	{
		if (rect.x == null) rect.x = new SVG.Length(0);
		if (rect.y == null) rect.y = new SVG.Length(0);
		if (rect.width == null) rect.width = new SVG.Length(0);
		if (rect.height == null) rect.height = new SVG.Length(0);
	...
	protected boolean startShape(SVG.TextPositionedContainer text, float x, float y)
	{
		if (text.x == null) text.x = new ArrayList<SVG.Length>();
		if (text.y == null) text.y = new ArrayList<SVG.Length>();
		if (text.x.size() == 0) text.x.add(new SVG.Length(0));
		if (text.y.size() == 0) text.y.add(new SVG.Length(0));
	...
	protected boolean changeShape(SVG.TextPositionedContainer text, float x, float y)
	{
		text.x.get(0).setValue(mRenderer, x);
		text.y.get(0).setValue(mRenderer, y);
		return true;
	...
	protected boolean stopShape(final SVG.TextPositionedContainer text, float x, float y)
	{
		if (text.getChildren().size() == 0 && mContext != null)
		{
			...
			builder.setPositiveButton("OK", new DialogInterface.OnClickListener()
			{
				...
				public void onClick(DialogInterface dialog, int which)
				{
					dialog.dismiss();
					try{mShape = mShape.clone();}catch(CloneNotSupportedException e){e.printStackTrace();}
					SVG.TextSequence string = new SVG.TextSequence(input.getText().toString());
					try{text.addChild(string);}catch(SAXException e){e.printStackTrace();}
					onTextChanged();
				}
			});
